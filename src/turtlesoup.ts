...
export function chordLength(radius: number, angleInDegrees: number): number {
  ...
  var pi = Math.PI;
  var radians =  angleInDegrees * (pi/180);
  let chordLength =  Math.round (2*radius* Math.sin(radians/2)*100)/100 ;
  
  
  return chordLength; // Placeholder - replace with your implementation
}
...
export function drawApproximateCircle(
  ...
  var degree : number = 360/numSides
  let x =  chordLength(radius , degree)
...
export function distance(p1: Point, p2: Point): number {
  ...
  var distancebetween = Math.sqrt(Math.pow(p1.x-p2.x , 2) +Math.pow(p1.y-p2.y , 2))
  return distancebetween; // Placeholder
}
...
export function findPath(turtle: Turtle, points: Point[]): string[] {
  ...
  for(let i=0 ; i<points.length ; i++){
    ...
    let forwarding:number =distance( current_position, points[i]);
    let turning:number = Math.atan2(points[i].x - current_position.x , points[i].y - current_position.y) * 180 /Math.PI;
    str += "forward "+forwarding + "turn"+turning ;
 ...
 * @param turtle The turtle to use.
 */
export function drawPersonalArt(turtle: Turtle): void {//drawin a sword dragonslayer from berserk
  // Draw the blade (long and wide) too big to be called a sword
  turtle.color("red");
  turtle.forward(220);
  turtle.turn(45);
  turtle.forward(35);
...
export function main(): void {
 ...
 //awApproximateCircle(turtle, 50, 360);
